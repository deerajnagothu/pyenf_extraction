...
video_folder = "Recordings/Deepfake4/"
video_rec_name = "original_face.mp4"
power_rec_name = "deepfake_power.wav"
...
def second_half_extract_row_pixel(frame):
    ...
    if frame_shape[2] == 3:  # its an RGB frame
        average_frame_across_rgb = np.mean(frame, axis=2)
        average_frame_across_column = np.mean(average_frame_across_rgb[:,1440:], axis=1)
    else:
...
fig, (video,deepfake, power) = plt.subplots(3, 1, sharex=True)
video.plot(ENF[:-12],'b')
video.set_title("ENF Signal from Video", fontsize=12)
...
row_signal = pickle.load(load_variable_file)
load_variable_file.close()
print("Loaded the Deepfake Row Signal. \n")
...
OurStripCell, initial_frequency = video_signal_object.compute_combined_spectrum(spectro_strip, weights,
                                                                                frequency_support)
deepfake_ENF = video_signal_object.compute_ENF_from_combined_strip(OurStripCell, initial_frequency)
deepfake.plot(deepfake_ENF[:-12],'r')
deepfake.set_title("Deepfake ENF Signal", fontsize=12)
deepfake.set_ylabel("Freq (Hz)", fontsize=12)
deepfake.set_xlabel("Time", fontsize=12)
"""
...
fig.tight_layout()
fig.show()
total_windows,rho = correlation_vector(ENF[:-7], power_ENF[:-7],window_size,shift_size)

# temp load of rho
"""
variable_location = video_folder + "deepfake_rho.pkl"
store_variable_file = open(variable_location, 'wb')
pickle.dump(rho, store_variable_file)
store_variable_file.close()

"""
variable_location = video_folder + "deepfake_rho.pkl"
load_variable_file = open(variable_location, 'rb')
deepfake_rho = pickle.load(load_variable_file)
load_variable_file.close()

print(rho)

rho = [[0.84514878, 0.87269013, 0.82095052, 0.93939458, 0.902144,   0.96282949,0.95840928]]
plt.figure()
t = np.arange(0,total_windows-1,1)
plt.plot(t,rho[0][1:],'g--', label="Original Video")
plt.plot(t,deepfake_rho[0][1:],'b', label="Deepfake Video")
plt.hlines(y=0.8, xmin=0, xmax=len(t), colors='r', linestyles='--', lw=2)
plt.ylabel('Correlation Coefficient', fontsize=12)
plt.xlabel('Number of Windows compared', fontsize=12)
plt.title('Deepfake Vs Original Video ENF Comparison', fontsize=12)
